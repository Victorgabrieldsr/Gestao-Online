<?php
    ...
    $stmt = $conn->prepare("INSERT INTO investimento.ativos(idUser, nomeAtivo, categoria, dataCompra, descricao, quantidade, precoUnitario, valorTotal, porcentagemAno, tipo, bola, dataComparacao) 
    VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
    $stmt->bind_param('issssidddsss', $_SESSION['idUser'], $nomeAtivo, $categoria, $datacompra, $descricao, $quantidade, $precounit, $valortotal, $porcentoAno, $tipo, $bola, $dataComparacao);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_insert = "INSERT INTO investimento.cotasAtivos (idUser, nomeAtivo, categoria, quantidade, precoUnitario) 
                    VALUES (?, ?, ?, ?, ?)";
